...
# Récupérer le chemin du fichier PDF depuis les arguments de la ligne de commande
if len(sys.argv) < 2:
    print("Usage: python process_pdf.py <pdf_file_path> [proximity_threshold]")
    sys.exit(1)

# Chemin du fichier PDF
file_path = sys.argv[1]

# Seuil de proximité pour la détection des rectangles (optionnel)
proximity_threshold = int(sys.argv[2]) if len(sys.argv) > 2 else 50

# Debug : afficher le chemin du fichier et le seuil de proximité
print(f"Processing PDF file: {file_path}")
print(f"Proximity threshold: {proximity_threshold}")
sys.stdout.flush()  # Forcer l'écriture immédiate dans la sortie standard

# Function to create a directory if it doesn't exist
def create_directory_for_pdf(pdf_filename):
    folder_name = os.path.splitext(os.path.basename(pdf_filename))[0]
    output_folder = os.path.join("uploads", folder_name)
